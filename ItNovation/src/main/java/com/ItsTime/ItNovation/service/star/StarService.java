...
public class StarService {
    ...
    public ResponseEntity singleStarEvaluate(SingleStarEvaluateDto singleStarEvaluateDto){
        try{
            User user = userRepository.findById(singleStarEvaluateDto.getUserId())
                    .orElseThrow(()-> new IllegalArgumentException("해당 유저가 존재하지 않습니다."));
            Movie movie = movieRepository.findById(singleStarEvaluateDto.getMovieId())
            ...
            Star build = Star.builder()
                    .user(user)
                    .movie(movie)
                    .score(singleStarEvaluateDto.getStarScore())
                    .build();
